      ...
      <div className={styles.content}>
        ...
        <section className={styles.section} id="auctions">
          ...
          <div className={styles.auctionsPanel}>
            ...
            <div className={styles.auctionTable}>{sellerAuctions.map((auction) => <article key={auction.id}><img src={auction.product?.image_url || ''} alt="" /><div><span className={`${styles.status} ${styles[auction.status?.toLowerCase()]}`}>{auction.status}</span><h3>{auction.product?.title || 'Untitled product'}</h3><p>{auction.product?.category_name || 'Uncategorized'} · {auction.product?.condition || 'Unknown condition'}</p></div><strong>{formatPrice(auction.highest_bid?.amount ?? 0)}</strong><span>{auction.bid_count || 0} bids</span><span>{formatDate(auction.end_time)}</span><button className="btn-outline" type="button" onClick={() => navigate(`/seller/auctions/${auction.id}`)}>View</button></article>)}</div>
